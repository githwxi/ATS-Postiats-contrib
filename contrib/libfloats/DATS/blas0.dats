...
blas$_alpha_0 (alpha, x) =
  gmul_val_val<a> (alpha, x)
implement{a}
blas$_alpha_1 (alpha, x, y) =
  gadd_val_val<a> (gmul_val_val<a> (alpha, x), y)
implement{a}
blas$_alpha_beta
  (alpha, x, beta, y) =
  gadd_val_val<a> (gmul_val_val<a> (alpha, x), gmul_val_val<a> (beta, y))
// end of [blas_alphabeta]

(* ****** ****** *)

implement{a}
blas_inner$fmul (x, y) = gmul_val_val<a> (x, y)
