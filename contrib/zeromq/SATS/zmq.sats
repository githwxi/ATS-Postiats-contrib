...
int
zmq_unbind
  (void *socket, const char *endpoint);
*)
fun
zmq_unbind
(
...
int
zmq_send
  (void *socket, void *buf, size_t len, int flags);
*)
fun
zmq_send
  {m:int}{n:int | n <= m}
...
viewtypedef zmqmsg_t = zmqmsg_viewt0ype
...
*)
fun zmq_msg_size (msg: &zmqmsg_t):<> size_t = "mac#%"

(*
void *zmq_msg_data (zmq_msg_t *msg);
*)
fun zmq_msg_data (msg: &zmqmsg_t):<> Ptr1 = "mac#%"

(*
int zmq_msg_more (zmq_msg_t *message);
*)
fun zmq_msg_more (msg: &zmqmsg_t):<> natLt(2) = "mac#%"
...
*)
fun zmq_msg_get
  (msg: &zmqmsg_t, property: int): int(*verr*) = "mac#%"
// end of [zmq_msg_get]

fun zmq_msg_get_exn
  (msg: &zmqmsg_t, property: int): intGt(0) // valid return value
...
int
zmq_msg_set
  (zmq_msg_t *message, int property, int value);
*)
fun
zmq_msg_set
(
  msg: &zmqmsg_t >> _, property: int, value: int
) : interr = "mac#%" // endfun

fun
zmq_msg_set_exn
  (msg: &zmqmsg_t >> _, property: int, value: int): void
...
fun zmq_msg_init
(
  msg: &zmqmsg_t? >> opt (zmqmsg_t, i==0)
) : #[i:int | i <= 0] int (i) = "mac#%"

fun zmq_msg_init_exn (msg: &zmqmsg_t? >> _): void
...
int
zmq_msg_init_size
  (zmq_msg_t *msg, size_t size);
*)
fun
zmq_msg_init_size
(
  msg: &zmqmsg_t? >> opt (zmqmsg_t, i==0), n: size_t
) : #[i:int | i <= 0] int (i) = "mac#%"

fun
zmq_msg_init_size_exn (msg: &zmqmsg_t? >> _, n: size_t): void
...
//
typedef
zmq_free_fn = (ptr(*data*), ptr(*hint*)) -> void
//
fun
zmq_msg_init_data
(
...
fun zmq_msg_close_exn (msg: &zmqmsg_t >> _?): void
...
fun
zmq_msg_copy
  (dst: &zmqmsg_t >> _, src: &zmqmsg_t): interr = "mac#%"
// end of [zmq_msg_copy]

fun
zmq_msg_copy_exn (dst: &zmqmsg_t >> _, src: &zmqmsg_t): void
...
fun zmq_msg_move_exn (dst: &zmqmsg_t >> _, src: &zmqmsg_t): void
...
zmq_msg_recv
(
  msg: &zmqmsg_t >> _, sock: !zmqsock1, flags: int
) : interr = "mac#%" // endfun
...
zmq_sendmsg
(
  sock: !zmqsock1, msg: &zmqmsg_t, flags: int
) : interr = "mac#%" // endfun
...
zmq_recvmsg
(
  sock: !zmqsock1, msg: &zmqmsg_t >> _, flags: int
) : interr = "mac#%" // endfun
